...
<span data-lang="en">
- It is possible to add worked hours for a specific employee (date, start time, end time).
- The system validates that the entered hours do not exceed 24 hours per day.
- A weekly report can be generated for each employee, including total hours.
- Data must be accessible only to authorized users (administrators and managers).
- Overtime (>40 hours/week) must be identified in the report.
</span>
...
<span data-lang="en">
- Employees cannot modify their own hours.
- Hours must be recorded in 24h format (HH:MM).
